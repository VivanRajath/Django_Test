...
# For development, make sure STATICFILES_DIRS is set correctly
STATICFILES_DIRS = [BASE_DIR / "static"]  # static folder in your project root
...
import os
from pathlib import Path
import dj_database_url  # we'll install dj-database-url for database config

BASE_DIR = Path(__file__).resolve().parent.parent

SECRET_KEY = os.getenv('SECRET_KEY', 'your-dev-secret-key')  
# Use an env var for production secret; fallback to a dev key.

DEBUG = True  # Set to False in production for security

ALLOWED_HOSTS = ['localhost', '127.0.0.1']
# For Render deployment, add your Render domain. Render provides it via env:
...
# Database configuration: SQLite for local dev, PostgreSQL for production via env
DATABASES = {
    'default': dj_database_url.config(
        default=f'sqlite:///{BASE_DIR / "db.sqlite3"}',
        conn_max_age=600   # persist database connections
    )
}

# Password validation – (use Django’s default validators, already in starter settings)
AUTH_PASSWORD_VALIDATORS = [
    {'NAME': 'django.contrib.auth.password_validation.UserAttributeSimilarityValidator'},
    {'NAME': 'django.contrib.auth.password_validation.MinimumLengthValidator'},
    {'NAME': 'django.contrib.auth.password_validation.CommonPasswordValidator'},
...
# Internationalization, timezone (default values can remain)
